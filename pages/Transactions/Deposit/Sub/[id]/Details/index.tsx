...
export default function DepositSubDetail() {
  const router = useRouter();
  const { id } = router.query;
  const debtor_sub_id = useStore((state: any) => state.debtor_sub_id);
  ...
  const fetchData = async () => {
    try {
      const url = `${config.API_HOST}/dashboards/company-wallet/main-agent-deposit-id-sub?company_id=${company.company_id}&agent_id=${id}&debtor_sub_id=${debtor_sub_id}`;
